...
def compute_c_nu():
    ...
    outputfile='E2W10Lz100K.csv'

    #output=openfile(outputfile)

    print(outputfile)
    datafile = cfg.datafilename(outputfile)
    ...
    L_list=np.unique(np.array(df['L'])).tolist()
    c_list=np.unique(np.array(df['c'])).tolist()
    W_list=np.unique(np.array(df['W'])).tolist()
